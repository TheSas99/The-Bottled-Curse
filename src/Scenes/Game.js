...
export default class GameScene extends Phaser.Scene {
  ...
  update () {
    ...
    //if spacebar is pressed fire bullets
    if (Phaser.Input.Keyboard.JustDown(this.spaceKey)) {
      // fire bullets in right direction
      this.bullets.fireBullet(this.player.x, this.player.y, this.player.direction);
      // add the bulletsound at a lower volume
      let bulletSound = this.sound.add('gun',{volume: 0.2});
    ...
    this.map.findObject('Player', (obj) => {
      if (this._NEWGAME && this._LEVEL === 1) {
        if (obj.type === 'StartingPosition') {
          ...
          let sound = this.sound.add('level', {loop: true, volume: 0.4});
          sound.play();
        }
      } else {
        this.player = new Player(this, obj.x, obj.y);
  ...
  loadNextLevel (endGame) {
    if (!this.loadingLevel) {
      ...
      let defeatSound = this.sound.add('defeated');
